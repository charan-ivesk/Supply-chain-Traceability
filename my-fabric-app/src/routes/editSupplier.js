...
router.post('/', async (req, res) => { 
  try {
    ...
    if (!supplier_id || !value) {
        return res.status(400).json({ error: 'supplier_id and new value are required in the request body' });
    }
    ...
    if (result1.length==0){
        ...
        return res.status(400).json({ error: 'supplier '+str+' has been deactivated' });
    }          

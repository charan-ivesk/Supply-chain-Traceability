...
router.post('/', async (req, res) => { 
  try {
    ...
    if (!facility_id || !value) {
        return res.status(400).json({ error: 'facility_id and value are required in the request body' });
    }
    ...
    if (result1.length==0){
        return res.status(400).json({ error: 'facility '+str+'does not exist' });
    }          
    ...
    console.log('facility has been edited');

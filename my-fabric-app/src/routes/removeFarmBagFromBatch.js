    ...
    try {
        ...
        for (var i=0;i<farmbaglist.length;i++){
            ...
            if (result.length==0){
                ...
                return res.status(400).json({ error: 'Farmbag '+str+' is not part of given batch' });
            }
            else if(result[0].value.batch_id!=batch_id){
                return res.status(400).json({ error: 'Farmbag '+str+' is not attached to given batch' });
            }
        ...
        if (input1.value.hasOwnProperty("farmerBag_ids")) {
            let filtered_FBList = input1.value.farmerBag_ids.filter(id => !farmbaglist.includes(id));
            console.log(filtered_FBList);
            input1.value.farmerBag_ids = filtered_FBList;
        }
        ...
        for (var i=0;i<farmbagdata.length;i++){
            ...
            farmbagdata[i].value.status="RECEIVED"

...
router.post('/', async (req, res) => { 
    try {
        ...
        if (!loc || !zfblist) {
            return res.status(400).json({ error: 'zeroflybaglist and Location are required in the request body' });
        }
        ...
        for (var i=0;i<zfblist.length;i++){
            ...
            const history=result[0].value.history
            history[history.length]=result[0].value.location
            result[0].value.history=history
            result[0].value.location=loc

            console.log("check")
            await contract.submitTransaction('writeData', str, JSON.stringify(result[0].value));
            let num= (i+1).toString()
            
            console.log(num +' ZeroFlyBag(s) updated')
        }

...
router.post('/', async (req, res) => {
  try {
    ...
    const employee_id=req.value.employee_id;
    const produce_id=req.body.produce_id;
    const value = req.body.value;

    if ( !value || !employee_id|| produce_id) {
        return res.status(400).json({ error:  'Employee_id, Poduce_id and Value are required in the request body' });
    }
    ...
    let type=value.grain
    const result = await contract.evaluateTransaction('queryValidFB', type);

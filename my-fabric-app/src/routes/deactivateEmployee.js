...
router.post('/', async (req, res) => { 
  try {
    ...
    if (!employee_id) {
        return res.status(400).json({ error: 'employee_id is required in the request body' });
    }
    ...
    if (result1.length==0){
        ...
        return res.status(400).json({ error: 'Employee '+str+' has already been deactivated' });
    }          
